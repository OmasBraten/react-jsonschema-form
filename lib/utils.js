...
function computeDefaults(schema, parentDefaults, formData, definitions) {
  if ("$ref" in schema) {
      ...
      case "array":
        {
          ...
          computed = new Array(length).fill(undefined).map(function (_, i) {
      ...
      case "object":
        {
          ...
          computed = formData ? mergeObjects(formData, defaultFilled) : defaultFilled;
          break;
        }
      default:
        {
    ...
    return schema.dependencies ? computeDefaults(retrieveSchema(schema, definitions, computed), undefined, computed, definitions) : computed;
  }
}
